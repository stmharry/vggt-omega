...
def patch_saliency_maps(
    ...
    for frame_index in range(frame_count):
        ...
        early_sources = q[early][:, source_indices].permute(1, 0, 2, 3, 4)
        deep_sources = k[deep][:, source_indices].permute(1, 0, 2, 3, 4)
        middle_sources = q[middle][:, source_indices].permute(1, 0, 2, 3, 4)
        early_similarity = temporal_similarity(q[early, frame_index], early_sources)
        deep_similarity = temporal_similarity(k[deep, frame_index], deep_sources)
        middle_variance = temporal_variance(q[middle, frame_index], middle_sources)
