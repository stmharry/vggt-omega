...
- `pipeline-memory-parallel`: runs aggregator blocks across ordered stage GPUs
  ...
  tensor size.
  `--query-parallel-devices 0,1,2,3` enables the prototype exact query-sharded
  path for aggregator global inter-frame attention: query rows are split across
  the listed visible CUDA devices, each shard attends to the full K/V sequence,
  and outputs are gathered in original sequence order.
